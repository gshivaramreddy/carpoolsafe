...
import sys, os
from api_client import post, get, is_logged_in, do_login, do_logout
...
h1,h2,h3,h4 { color:#e8f4fd !important; }
...
.gcard-glow {
  background:linear-gradient(135deg,#0d1f38,#0a172d);
  border:1px solid rgba(0,210,255,0.35); border-radius:20px;
  padding:22px; margin-bottom:14px;
  box-shadow:0 0 30px rgba(0,210,255,0.07);
}
.gradient-text {
...
.section-label {
  font-size:11px; font-weight:600; color:#4a7a98;
  text-transform:uppercase; letter-spacing:0.1em; margin-bottom:10px;
}
.stat-row {
  display:flex; justify-content:space-between; align-items:center;
  padding:8px 0; border-bottom:1px solid #0f2040; font-size:13px;
}
...
.live-pill {
  ...
  border-radius:999px; padding:4px 12px; font-size:12px; color:#00e676;
}
.live-dot {
...
.msg-me {
  background:linear-gradient(135deg,#1a2f50,#142440);
  border:1px solid rgba(0,210,255,0.15);
  border-radius:14px 14px 2px 14px; padding:9px 14px; margin:4px 0; font-size:13px;
}
.msg-other {
  background:#0d1f38; border:1px solid #1a3a5c;
  border-radius:14px 14px 14px 2px; padding:9px 14px; margin:4px 0; font-size:13px;
}
.msg-system {
  background:rgba(123,47,247,0.07); border:1px solid rgba(123,47,247,0.15);
  border-radius:8px; padding:5px 12px; text-align:center;
  font-size:11px; color:#6b8fa8; margin:4px 0;
}
.invite-code {
  font-family:monospace; font-size:26px; font-weight:800; letter-spacing:8px;
  color:#00d2ff; text-align:center; padding:14px; background:#0d1f38;
  border:2px dashed #1a3a5c; border-radius:14px; margin:10px 0;
}
.upi-box {
  background:rgba(0,230,118,0.06); border:2px dashed rgba(0,230,118,0.25);
  border-radius:14px; padding:16px; text-align:center;
  font-family:monospace; color:#00e676; word-break:break-all; margin:10px 0;
}
.sos-wrap .stButton>button {
  background:linear-gradient(135deg,#ff416c,#ff0040) !important;
  font-size:18px !important; height:80px !important;
  border-radius:18px !important;
  box-shadow:0 0 50px rgba(255,65,108,0.4) !important;
}
</style>
""", unsafe_allow_html=True)
...
def render_auth():
    ...
    <div style='text-align:center;padding:24px 0 20px;'>
      ...
      <div style='color:#4a7a98;font-size:13px;margin-top:8px;line-height:1.6;'>
        AI ride matching · Real-time GPS · Women safety · UPI payments
      </div>
      <div style='display:flex;gap:7px;justify-content:center;flex-wrap:wrap;margin-top:12px;'>
    ...
    with tab_login:
        ...
        <div style='text-align:center;margin-top:14px;font-size:13px;color:#4a7a98;'>
          No account? Click the
          <b style='color:#00d2ff;'>Create Account</b> tab above ↑
        ...
        if st.button("Create Account  →", key="btn_register", use_container_width=True):
            ...
            if err:
                ...
                with st.spinner("Creating your account..."):
                    result = post("/auth/signup", payload, auth=False)
                ...
                if result:
                    ...
                    st.session_state.user_gender = rg_gender
...
def render_dashboard():
    ...
    <h1 style='font-size:2.1rem;margin-bottom:4px;'>
      {greet}, <span class='gradient-text'>{name}</span> 👋
    </h1>
    ...
    for col, (icon, title, sub, color, bg, page) in zip(
        [c1, c2, c3, c4], [
        ...
        with col:
            st.markdown(f"""
            <div style='background:{bg};border:1px solid {color}33;border-radius:16px;
                        padding:18px;text-align:center;margin-bottom:8px;'>
              <div style='font-size:30px;'>{icon}</div>
              <div style='font-weight:700;margin:8px 0 4px;color:{color};font-size:14px;'>
                {title}
              </div>
    ...
    m1.metric("🚗 Rides Created", len(rides),
              delta=f"{len([r for r in rides if r.get('status')=='scheduled'])} scheduled")
    m2.metric("🎟️ My Bookings",  len(bookings),
              delta=f"{len([b for b in bookings if b.get('status')=='confirmed'])} active")
    m3.metric("👥 Groups",        len(groups))
    m4.metric("💳 Total Paid",    f"₹{paid:.0f}")
        ...
        if items:
            for item in items:
                if "source_address" in item:
                    ...
                    sc = {"scheduled":"badge-cyan","active":"badge-green",
                          "completed":"badge-purple","cancelled":"badge-red"}.get(item.get("status",""), "badge-blue")
                else:
                    lbl = f"{item['pickup_address'][:20]}… → {item['drop_address'][:20]}…"
                    meta = f"💺 {item['seats_booked']} seat(s) · ₹{item.get('estimated_price',0):.0f}"
                    sc = {"confirmed":"badge-green","cancelled":"badge-red",
                          "completed":"badge-cyan"}.get(item.get("status",""), "badge-amber")
                st.markdown(f"""
                <div class='gcard' style='padding:14px;margin-bottom:8px;'>
                  <div style='display:flex;justify-content:space-between;align-items:center;'>
                    ...
                    <span class='badge {sc}'>{item.get("status","").upper()}</span>
                  </div>
                </div>""", unsafe_allow_html=True)
        else:
            st.markdown("""
    ...
    with col_r:
        ...
        try:
            ...
            for nm, lat, lng, color in [
                ("HITEC City",    17.4435, 78.3772, "#00d2ff"),
                ("Charminar",     17.3616, 78.4747, "#ff6b9d"),
                ("Gachibowli",    17.4401, 78.3489, "#7b2ff7"),
                ("Banjara Hills", 17.4165, 78.4480, "#ffc107"),
                ("Secunderabad",  17.4399, 78.4983, "#00e676"),
                ("Madhapur",      17.4478, 78.3909, "#42a5f5"),
                ("LB Nagar",      17.3469, 78.5528, "#ff9800"),
                ("Kukatpally",    17.4849, 78.4138, "#ff416c"),
            ]:
                folium.CircleMarker(
                    [lat, lng], radius=8, color=color, fill=True,
